...
def p_expression_binop(prod):
    ...
    prod[0] = shared.node_factory.create_node(ast.BinOp, children=[prod[1], prod[3]], leaf=prod[2])
...
def p_expression_relop(prod):
    ...
    prod[0] = shared.node_factory.create_node(ast.RelOp, children=[prod[1], prod[3]], leaf=prod[2])


def p_expression_uminus(prod):
    "expression : MINUS expression %prec UMINUS"
    prod[0] = shared.node_factory.create_node(ast.UnaryOp, children=[prod[2]], leaf="-")
...
def p_expression_number(prod):
    "expression : NUMBER"
    prod[0] = shared.node_factory.create_node(ast.Float, leaf=prod[1])
...
def p_expression_bool(prod):
    """expression : TRUE
    | FALSE"""
    prod[0] = shared.node_factory.create_node(ast.Bool, leaf=shared.reserved_words[prod[1]])


def p_expression_deref(prod):
    "expression : DEREF"
    prod[0] = shared.node_factory.create_node(ast.Deref, leaf=prod[1][1:])


def p_expression_string_literal(prod):
    "expression : STRINGLITERAL"
    prod[0] = shared.node_factory.create_node(ast.StringLiteral, leaf=prod[1][1:])
